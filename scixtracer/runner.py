...
def __values_inputs(args_inputs: dict,
                    data_info: list[DataInfo] | DataInfo) -> list:
    ...
    for i, value in args_inputs.items():
        if value["type"] == "query":
            out.append(data_inf[next_idx].uri)
            next_idx += 1
        else:
...
class BatchItem:
    ...
    def __init__(self, func: Callable, inputs: list, outputs: list[str]):
        self.func = func
        self.inputs = inputs
...
def __batch_job(dataset, job: Job, run_id: str, job_id: str):
    ...
    for d_info in data_info:
        ...
        for i, out_type in enumerate(out_types):
            ...
            out_uris.append(out_info.uri.value)
